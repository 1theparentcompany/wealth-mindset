...
window.toggleMetadataList = function () {
    // No longer needed for overlay but kept for compatibility or used if we add a mobile toggle
...
window.renderMetadataPreview = function (itemId, activeTabName = "About") {
    ...
    if (!settings.stats) {
        settings.stats = (item.type === 'book' || !item.type) ? [
            { label: 'Avg Rating', value: `⭐ ${item.rating || '4.8'} / 5.0` },
            { label: 'Likes Score', value: `👍 ${item.likes_percent || '85'}% Liked` },
            { label: 'Reviews', value: `💬 ${item.reviews_count || '120'} Reviews` },
            { label: 'Chapter Count', value: `📖 ${item.chapters_count || (Array.isArray(item.chapters) ? item.chapters.length : '12')} Chapters` }
        ] : [
            { label: 'Read Time', value: `⏱️ ${item.read_time || '5 min'}` },
            { label: 'Source', value: `🔗 ${item.original_source || 'Original'}` },
    ...
    container.innerHTML = `
                ...
                <div style="display: flex; gap: 45px; position: relative; z-index: 2; align-items: center;">
                    ...
                    <div style="flex: 1;">
                        ...
                        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 25px; font-size: 1.15rem; color: #94a3b8;">
                            ...
                            <span>•</span>
                            <span style="color: #fbbf24; font-weight: 700;">⭐ ${item.rating || '4.8'} / 5.0</span>
                        ...
                        <div style="display: flex; gap: 20px; margin-bottom: 35px;">
                            ...
                            ${item.type === 'book' || !item.type ? `
                                <div style="display: flex; flex-direction: column; gap: 5px;">
                                    <div style="font-size: 1.5rem; font-weight: 800; color: #fff;">${item.chapters_count || (Array.isArray(item.chapters) ? item.chapters.length : '12')}</div>
                                ...
                                <div style="width: 1px; background: rgba(255,255,255,0.1);"></div>
                                <div style="display: flex; flex-direction: column; gap: 5px;">
                                    <div style="font-size: 1.5rem; font-weight: 800; color: #fff;">${settings.pages || '350'}</div>
                            ...
                            ` : `
                                <div style="display: flex; flex-direction: column; gap: 5px;">
                                    <div style="font-size: 1.5rem; font-weight: 800; color: #fff;">${settings.readTime || '5 min'}</div>
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                                ${item.type === 'book' || !item.type ? `
                                    <div>
                                        ...
                                        <div onclick="editMetadataField('${item.id}', 'detailSettings.publisher', 'Publisher Name')" style="color:#e2e8f0; font-size:0.9rem; cursor:pointer;">${settings.publisher || 'Not Set'} ✎</div>
                                    </div>
                                    <div>
                                        <label style="display:block; color:#64748b; font-size:0.7rem; font-weight:800; margin-bottom:4px;">ISBN / ID</label>
                                        <div onclick="editMetadataField('${item.id}', 'detailSettings.isbn', 'ISBN')" style="color:#e2e8f0; font-size:0.9rem; cursor:pointer;">${settings.isbn || 'Not Set'} ✎</div>
                                    </div>
                                    <div>
                                        <label style="display:block; color:#64748b; font-size:0.7rem; font-weight:800; margin-bottom:4px;">PUBLICATION YEAR</label>
                                        <div onclick="editMetadataField('${item.id}', 'detailSettings.pubYear', 'Year')" style="color:#e2e8f0; font-size:0.9rem; cursor:pointer;">${settings.pubYear || '2024'} ✎</div>
                                    </div>
                                ` : `
                                    <div>
        ...
        return `
                ...
                <div style="margin-top: 50px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
                        <h3 style="font-size: 1.4rem; font-weight: 800; margin: 0; color: #fff;">${activeTabName} Sections</h3>
            ...
            settings.tabContent[activeTabName].map((sec, idx) => `
                        ...
                        `).join('')
            :
            ((activeTabName !== 'About' && activeTabName !== 'Author') ?
